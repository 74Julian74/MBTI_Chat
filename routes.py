...
def register_routes(app, socketio):
    ...
    def analyze_emotion():
        try:
            ...
            if not messages:
                return jsonify({
                    ...
                    'emotions': {
                        ...
                        'sadness': 0, 'surprise': 0, 'happiness': 0
                    },
            ...
            emotion_result = emotion_predictor.predict_single(last_message)
            ...
            if isinstance(emotion_result, dict):
                ...
                # 如果不是字典，嘗試從 _format_response 結果中提取情緒數據
            ...
            try:
                ...
                try:
                    ...
                    current_app.logger.error(f"解析主導情緒時出錯: {str(e)}")
                ...
                chinese_emotion = emotion_names.get(dominant_emotion_name, dominant_emotion_name)
                ...
                return jsonify({
                    ...
                    'dominant_emotion': {
                        ...
                        'score': dominant_emotion_score
                    },
                    'person_emotion': f"{chinese_emotion} ({dominant_emotion_score * 100:.1f}%)",
                    'emotion_reason': f"檢測到主要情緒為{chinese_emotion}，信心度為 {dominant_emotion_score * 100:.1f}%",
                    'mbti_explanation': get_mbti_explanation(opponent_info['mbti'])
                })
            except Exception as e:
                app.logger.error(f"處理情緒分數時出錯: {str(e)}")
                return jsonify({
                    'name': opponent_info['name'],
                    'mbti': opponent_info['mbti'],
                    'emotions': emotion_scores,
                    'person_emotion': 'Unknown (0%)',
                    'emotion_reason': '無法分析情緒強度',
                    'mbti_explanation': get_mbti_explanation(opponent_info['mbti'])
                })
