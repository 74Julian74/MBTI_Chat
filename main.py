...
class EmotionPredictor:
    ...
    def predict_message(self, text):
            results = {}
            for emotion in self.emotions:
                encoding = self.tokenizers[emotion].encode_plus(
                    text,
                    add_special_tokens=True,
                    max_length=128,
                    padding='max_length',
                ...
                with torch.no_grad():
                    outputs = self.models[emotion](input_ids=input_ids, attention_mask=attention_mask)
                    probabilities = torch.softmax(outputs.logits, dim=1)
                    positive_prob = probabilities[0][1].item()
                
                results[emotion] = positive_prob
